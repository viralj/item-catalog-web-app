...
from flask import Flask, render_template, \
    url_for, request, redirect, flash, jsonify, make_response
...
CLIENT_ID = json.loads(
    open(os.path.join(__location__, 'client_secret.json'),
...
def gconnect():
    ...
    try:
        ...
        oauth_flow = flow_from_clientsecrets(
            os.path.join(__location__, 'client_secret.json'), scope='')
...
def showCategory(category_name):
    ...
    items = session.query(Items) \
        .filter_by(category=category).order_by(asc(Items.name)).all()
...
def allItemsJSON():
    ...
    for c in range(len(category_dict)):
        items = [i.serialize for i in session.query(Items).filter_by(
...
if __name__ == '__main__':
    app.secret_key = 'SHhhH_ThisIS-S3cR3t'
    app.debug = True
